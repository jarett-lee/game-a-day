...
class MyBigInt {
    ...
    toString() {
        ...
        for (let i = this.numbers.length - 2; i >= 0; i--) {
            ...
            // @ts-ignore
            if (x)
                out = out + x.toString().padStart(MyBigInt.DIGIT_LEN, "0");
            else
                out = out + MyBigInt.DIGIT_VAL.toString().slice(1);
        }
        return out;
    }
    static abs(a) {
        const out = new MyBigInt(a);
        out.sign = Math.abs(out.sign);
        return out;
    }
    static add(a, b) {
        if (a.sign === 0)
    ...
    static shift(a, b) {
        ...
        let bSign;
        if (typeof b === "number") {
            if (!Number.isInteger(b) || !(b <= Number.MAX_SAFE_INTEGER))
        ...
        if (bSign === -1) {
            ...
            for (let i = a.numbers.length; i >= macroOffset; i--) {
                ...
                if (microOffset > 0) {
                    ...
                    console.log(top, bot);
                    if (top !== 0)
                        out.numbers[i - macroOffset] = (out.numbers[i - macroOffset + 1] || 0) + top;
                    if (bot !== 0 && i - macroOffset - 1 >= 0)
                        out.numbers[i - macroOffset - 1] = (out.numbers[i - macroOffset] || 0) + bot;
                }
                else {
                    if (x !== 0)
                        out.numbers[i - macroOffset] = x;
                }
            }
            ...
            if (macroOffset + a.numbers.length > Number.MAX_SAFE_INTEGER)
                throw new Error("Shift too large");
            ...
            for (let i = a.numbers.length; i >= 0; i--) {
                ...
                if (microOffset > 0) {
                    ...
                    if (top !== 0)
                        out.numbers[i + macroOffset + 1] = (out.numbers[i + macroOffset + 1] || 0) + top;
                    if (bot !== 0)
                        out.numbers[i + macroOffset] = (out.numbers[i + macroOffset] || 0) + bot;
                }
                else {
                    if (x !== 0)
                        out.numbers[i + macroOffset] = x;
...
MyBigInt.DIGIT_LEN = 14;
MyBigInt.DIGIT_VAL = 1e14;

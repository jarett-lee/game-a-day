...
const canv: HTMLCanvasElement = canvUnsafe as HTMLCanvasElement;

const ctxUnsafe = canv.getContext("2d");
if (!ctxUnsafe) throw new Error("Unexpected null or undefined value");
const ctx = ctxUnsafe as CanvasRenderingContext2D;
  ...
  draw() {
    if (this.frame > this.count) {
      ...
      drawRect2(
        this.x,
        this.y,
        Block.OFFSET * Math.abs(Math.sin((Math.PI * this.frame) / this.count))
      );
    }
  }
}
...
function createRect(x: number, y: number, initFrame: number, count: number) {
...
function drawRect2(x: number, y: number, offset: number) {
  ctx.beginPath();
  ctx.rect(
    x * SIZE + 0.5 + offset / 2,
    y * SIZE + 0.5 + offset / 2,
